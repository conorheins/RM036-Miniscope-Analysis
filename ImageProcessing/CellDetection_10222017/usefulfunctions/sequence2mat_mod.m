...
% edit 1.5, Conor Heins, 10222017 -- fixed bug for case when Tchunk is greater
% than number of total frames in data folder
...
Tchunk = 40000; % 10172017 -- edit for large computer to read in all data at once, changed to 40,000 frames (half of all frames)
...
if size(firstChunk,3) ~= length(firstFrame:(firstFrame+Tchunk-1))
    Tchunk = size(firstChunk,3);
end
...
%     data.Yfilt(:,firstFrame:(firstFrame+Tchunk-1)) = reshape(temp,d1*d2,[]); % edit 1.3 08192017 -- initialize Y to 2D shape, makes calling it easier
    data.Yfilt(:,:,firstFrame:(firstFrame+Tchunk-1)) = temp; % edit 1.4 10172017 -- back to old version, initialie Y to 3D shape, makes cell detection easier later on
end
