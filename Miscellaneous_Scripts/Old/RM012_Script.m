...
if strcmp(trial_based,'y')
    fprintf('\nThe data has been mapped to the hard disk. It has %d X %d pixels X %d frames, and is comprised of %d total trials. \nLoading all data requires %.2f GB RAM\n\n', ...
        d1, d2, numFrames,length(unique(frameIndices(:,1))),prod(Ysiz)*8/(2^30));
else
     fprintf('\nThe data has been mapped to hard disk. It has %d X %d pixels X %d frames. \nLoading all data requires %.2f GB RAM\n\n', ...
...
min_pixel = 5;      % minimum number of nonzero pixels for each neuron, in terms of the downsampled data
...
options.min_corr = min_corr; options.min_pnr = min_pnr; options.min_pixel = min_pixel; 
options.bd = bd; options.nk = 1; neuron_full.options = options;
...
while miter <= maxIter
    ...
    for m=1:5    
        ...
        neuron.trimSpatial(0.05, 5); % for each neuron, apply imopen first and then remove pixels that are not connected with the center
...
neuron.Coor = plot_contours(neuron.A, Cnn, 0.5, 0, [], [], 2);
colormap winter;
axis equal; axis off;
...
saveName = [fnam(1:end-4) '_Trial1Results.mat'];
globalVars = who('global');
eval(sprintf('save %s %s', saveName, strjoin(globalVars)));
