...
%% load previous sessions' results (if existing) and append most recent results

% tic
% Rat8 = cell(2,7);
% Rat8{1,1} = 'SA1';
% Rat8{2,1} = struct('A',sparse(A),'C',C);
% save(fullfile(fdir,'Rat8_results.mat'),'Rat8')
% fprintf('Time taken to create new results_struct and save it: %.2f seconds\n',toc)

tic
load(fullfile(fdir,'Rat20_results.mat'))
Rat20{1,7} = 'Reinstatement';
Rat20{2,7} = struct('A',sparse(A),'C',C);
save(fullfile(fdir,'Rat20_results.mat'),'Rat20','-v7.3')
fprintf('Time taken to load old analysis and append results: %.2f seconds\n',toc)

%% view spatial components alongside traces
% for i = 1:numROIs; 
%     subplot(121); 
%     ai = reshape(A(:,i),d1,d2);
%     r = round(cell_coords(i,1)); c = round(cell_coords(i,2));
%     imagesc(ai((r-10):(r+10),(c-10):(c+10)));
%     subplot(122);
%     plot(C(i,:)); 
%     pause;
% end
