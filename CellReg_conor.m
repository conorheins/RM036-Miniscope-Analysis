%% example cell reg demo script

input_dir = 'NeuralData/PostMerge';

all_files = subdir(fullfile(input_dir,'*.mat'));
all_files = {all_files(:).name};

temp = all_files{6}; % index of all_files tells you which rat to register
cell_reg_fnams = convert_to_cellreg(temp,[],[400 400],'CellReg-1.3.4/data');
...
results_directory=sprintf('CellReg-1.3.4/data/Rat%d_data/Results',ratID);
...
microns_per_pixel=2.75;

% Loading the data:
disp('Stage 1 - Loading sessions')
[spatial_footprints,number_of_sessions]=load_multiple_sessions(cell_reg_fnams);
...
maximal_distance=24; % cell-pairs that are more than 12 micrometers apart are assumed to be different cells
...
%         initial_threshold=spatial_correlation_intersection; % the threshold for p_same=0.5;
        initial_threshold = 0.7; % used for Rat8
    else
...
%         initial_threshold = 15; used particularly for registering SA1 to
%         Ext4 for Rat15
    else
...
registration_approach='Simple threshold'; % used for Rat8
...
%             final_threshold=spatial_correlation_intersection; % the threshold for p_same=0.5;
            final_threshold = 0.7; % used for Rat8
        else
