function [M_final,shifts_g,template] = normcorre_batch_onFilt(Y,options,template)
...
% Conor edit 08202017 to allow 2D input data
...
elseif isobject(Y);
    filetype = 'mem';
    sizY = size(Y,'Yfilt'); % Conor edit way back when
    T = sizY(end);
...
switch filetype
    ...
    case 'mem' % Conor edit 08202017, first condition where nd == 1
        if nd == 1; Y_temp = Y.Yfilt(:,1:init_batch); Y_temp = reshape(Y_temp,options.d1,options.d2,[]); nd = 2; elseif nd == 2; Y_temp = Y.Yfilt(:,:,1:init_batch); elseif nd == 3; Y_temp = Y.Yfilt(:,:,:,1:init_batch); end
    case 'mat'
...
sizY = [d1,d2]; % Conor edit 08202017
if nd == 2; d3 = 1; end
...
%         if nd == 2; M_final.Yfilt(d1*d2,T) = zeros(1,data_type); end % Conor edit 08202017 so that there's no reshaped copy, original is reshaped
        if nd == 2; M_final.Yfilt(d1,d2,T) = zeros(1,1,data_type); end % Conor edit 10172017 no more reshaping, only full tensor-shaped copy
        if nd == 3; M_final.Yfilt(d1*d2*d3,T) = zeros(1,data_type); end
...
for it = 1:iter
    for t = 1:bin_width:T
        switch filetype
            ...
            case 'mem'
%                 if nd == 2; Ytm = single(Y.Yfilt(:,t:min(t+bin_width-1,T))); Ytm = reshape(Ytm,d1,d2,[]); end % Conor edit 0820217 for reshaped input
                if nd == 2; Ytm = single(Y.Yfilt(:,:,t:min(t+bin_width-1,T))); end % Conor edit 10172017 expects 3D shaped input
                if nd == 3; Ytm = single(Y.Yfilt(:,:,:,t:min(t+bin_width-1,T))); end
            case 'mat'
...
%                     if nd == 2; M_final.Yfilt(:,t+lY-rem_mem:t+lY-1) =
%                     cast(reshape(mem_buffer(1:d1*d2*d3*rem_mem),d1*d2*d3,rem_mem),data_type);
%                     end % Conor edit 0820217 removing Yr, since M_final.Yfilt is already reshaped
                    if nd == 2; M_final.Yfilt(:,:,t+lY-rem_mem:t+lY-1) = cast(mem_buffer(:,:,1:rem_mem),data_type); end % Conor edit 10172017, expects everything 3D                       
                    if nd == 3; M_final.Yfilt(:,:,:,t+lY-rem_mem:t+lY-1) = cast(mem_buffer(:,:,:,1:rem_mem),data_type); end
