...
% Eftychios A. Pnevmatikakis, Simons Foundation, 2015
    ...
    defoptions.nrgthr = 0.8;              % energy threshold (default 0.8) % Conor default edit 08172017
    defoptions.clos_op = strel('square',3);  % morphological operator for closing
    defoptions.medw = [3,3];                 % size of median filter
    defoptions.nb = 0;                       % number of background components (default: 0) % Conor default edit 08172017
    ...
    if ~isfield(options,'nb') || isempty(options.nb); options.nb = defoptions.nb; end % Conor edit 08092017 -- ask PC about this
