...
if ~isfield(params,'se')  || isempty(params.se)  % morphological element (for 'dilate' and 'pos_pix')
...
elseif strcmpi(method,'pos_pix'); method = 'pos_pix';
...
switch method 
    ...
    case 'pos_pix' % Conor edit 08092017
        parfor i = 1:nr
            A_temp = imdilate(reshape(A(:,i),d1,d2,d3),expandCore);
            IND(:,i) = A_temp(:)>0;
        end
    otherwise
        IND = true(d,nr);
